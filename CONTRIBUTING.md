...
Install Rust via rustup (1.84+) and add the Soroban target:
bash
...
Making changes
Open an issue first for anything beyond a small fix, so we can discuss the approach before you put in the work.
One logical change per commit, with a clear commit message (type(scope): description, e.g. fix(escrow): correct fee rounding).
Every new function needs at least one passing-case test and one failing-case test.
Run cargo test and stellar contract build locally before opening a PR — CI will also run these, but catching it locally is faster for you.
Open PRs against main. Direct pushes to main are disabled.
Code style
No unwrap() outside test code.
No floating-point math — all amounts are i128, fee math uses basis points with checked_mul/checked_div.
